...
app.get('/movies/popular', async (req, res) => {
  try {
    ...
    const message = error.response?.data?.status_message || "Failed to fetch popular movies from TMDB.";
...
app.get('/movies/search', async (req, res) => {
  try {
    ...
    const status = error.response?.status || 500;
    const message = error.response?.data?.status_message || "Failed to search movies from TMDB.";
    res.status(status).json({ error: message });
  }
});

// Keep existing posts endpoints for backward compatibility
app.get('/posts', (req, res) => {
  res.json({ message: "GET /posts route active" });
});
